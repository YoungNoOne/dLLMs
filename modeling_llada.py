...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        return_attn_weights: bool = False,  # 新增：是否回传注意力权重
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]], Optional[torch.Tensor]]:
        """
        Compute attention outputs. If `return_attn_weights=True`, also return attention probabilities
        with shape (B, n_heads, T, S). 这里保持与 MDM 的改动一致：不使用因果 mask、不叠加 attention_bias。
        """
        ...
        if return_attn_weights:
            # 仅当用户请求时才额外计算注意力概率，避免无谓开销。
            # 这里手工计算 softmax(QK^T / sqrt(d))，与上面的输出路径在 eval 模式下一致。
            with torch.autocast(q.device.type, enabled=False):
                q_f = q.to(torch.float32)
                k_f = k.to(torch.float32)
                scale = 1.0 / math.sqrt(q_f.size(-1))
                # (B, nH, T, S)
        ...
        return self.attn_out(att), present, attn_probs
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def forward(
        ...
        layer_past: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        use_cache: bool = False,
        output_attentions: bool = False,  # 新增
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]], Optional[torch.Tensor]]:
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache, attn_probs = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache,
                return_attn_weights=output_attentions
            )
        else:
            att, cache, attn_probs = self.attention(
...
class LLaDAOutput(NamedTuple):
    ...
    """
    (Optional) Per-layer attention probabilities with shape `(batch_size, n_heads, tgt_len, src_len)`.
    Returned only when `output_attentions=True`.
    """
...
class LLaDABlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        all_attns: Optional[List[torch.Tensor]] = [] if output_attentions else None
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                # shape: (batch_size, seq_len, d_model)
                x, cache, att = self._activation_checkpoint_fn(  # type: ignore
            ...
            if all_attns is not None:
                all_attns.append(att)
        return x, attn_key_values, all_attns
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        Added `output_attentions` to optionally return per-layer attention weights.
        """
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache, group_attns = block_group(
                ...
                if all_attentions is not None and group_attns is not None:
                    all_attentions.extend(group_attns)
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model.forward(
            ...
            output_attentions=output_attentions,  # 新增
        )

        logits = outputs.logits
        hidden_states = outputs.hidden_states
        attentions = outputs.attentions  # 新增
        ...
        return CausalLMOutputWithPast(
            ...
            attentions=attentions,  # HuggingFace 的标准字段
        )
